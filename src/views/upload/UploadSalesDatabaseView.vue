<template>
  <SidebarLayout title="Upload Database Sales">
    <div class="upload-page">
      <div class="upload-card">
        <h2 class="card-title">Upload Database Sales</h2>
        <p class="card-subtitle">
          Satu file <strong>.xlsx</strong>. Database sales terbaru menjadi <strong>active</strong>;
        ...
        >
          ...
          {{ uploading ? 'Mengupload...' : 'Upload Database Sales' }}
        </button>
        ...
        <div v-if="result" class="success-card">
          <div class="success-title">✓ Database sales "{{ result.filename }}" tersimpan!</div>
          <div class="status-row">
            Status: <span class="status-badge badge-green">✓ Active</span>
          </div>
          <p class="success-hint">
            Lihat di <RouterLink to="/dashboard/sales-database">Database Sales</RouterLink>.
          </p>
        </div>
      </div>
    </div>
...
async function uploadFile() {
  ...
  try {
    ...
    const res = await apiClient.post('/upload_sales_database', form, {
    ...
    if (e.response?.status === 422) {
      ...
      uploadError.value = 'Akses hanya untuk Sales Agent.'
    } else {
